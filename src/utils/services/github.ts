...
// In-memory cache for user and repo details (session scoped)
export const userCache = new Map<string, any>();
export const repoCache = new Map<string, any>();
...
export async function fetchPullRequestMetrics(
  ...
  // Parallelize authored and reviewed PRs fetch
  const [authored, reviewed] = await Promise.all([
    ...
    octokit.rest.search.issuesAndPullRequests({
      q: `is:pr reviewed-by:${user.login}`,
      per_page: 100,
    }),
  ]);
  ...
  // Batch GraphQL queries for PR details (max 50 per batch)
  const items = Array.from(allItems.values());
  const batchSize = 20; // GitHub GraphQL allows large queries, but keep it safe
  const prDetails: any[] = [];
  for (let i = 0; i < items.length; i += batchSize) {
    ...
    const prData = await octokit.graphql<any>(query);
  ...
  // Parallelize commit fetches, but limit concurrency
  const limit = 5;
  ...
  for (let i = 0; i < prDetails.length; i += limit) {
    ...
    const chunkResults = await Promise.all(
      chunk.map(async ({ pr, item, owner, repo }) => {
        // Memoize commits per PR in repoCache
        ...
        const firstCommitAt = (commits as CommitType[]).reduce(
          (earliest: string | null, c: CommitType) => {
...
export async function fetchDeveloperMetrics(
  ...
  try {
    ...
    if (!user) {
      try {
        user = (await octokit.rest.users.getByUsername({ username: login })).data;
        userCache.set(login, user);
      } catch (err) {
        // Do not cache undefined, propagate the error
        throw err;
      }
    }
    if (!user) {
    ...
    if (!authored || !authored.data || !Array.isArray(authored.data.items)) {
      throw new Error('Failed to fetch authored PRs');
    }
    if (!reviewed || !reviewed.data || !Array.isArray(reviewed.data.items)) {
      throw new Error('Failed to fetch reviewed PRs');
    }
    ...
    // Batch GraphQL queries for PR details
    const items = authored.data.items;
    const batchSize = 20;
    for (let i = 0; i < items.length; i += batchSize) {
      const batch = items.slice(i, i + batchSize);
      const queries = batch
...
export async function fetchRepoInsights(
  ...
  if (!repoData) {
    repoData = await octokit.rest.repos.get({ owner, repo });
    repoCache.set(repoKey, repoData);
  }
...
export const __getUserCache = () => userCache;
export const __getRepoCache = () => repoCache;
