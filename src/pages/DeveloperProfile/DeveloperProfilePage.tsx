...
import type { DeveloperMetrics } from '../../utils/services/github';
...
export default function DeveloperProfilePage() {
	...
	useDocumentTitle(
		data ? `${data.name || data.login} - Developer Insights` : 'Developer Insights'
	);
						...
						<div className="grid gap-6 md:grid-cols-3">
							{METRIC_INFO.map((info) => (
								<DeveloperMetricCard
									...
									value={
										data
											? (data[info.valueKey as keyof DeveloperMetrics] as number)
											: 0
									}
									format={info.format}
								/>
